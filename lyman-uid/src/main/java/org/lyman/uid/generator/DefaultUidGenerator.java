 ...
 */
package org.lyman.uid.generator;

import org.lyman.exceptions.LymanErrorCode;
import org.lyman.exceptions.LymanException;
import org.lyman.uid.BitsAllocator;
import org.lyman.uid.UidGenerator;
import org.lyman.uid.utils.DateUtils;
import org.lyman.uid.worker.WorkerIdAssigner;
...
public class DefaultUidGenerator implements UidGenerator, InitializingBean {
    ...
    @Override
    public long getUID() throws LymanException {
        try {
            return nextId();
        } catch (Exception e) {
            LOGGER.error("Generate unique id exception. ", e);
            throw new LymanException(LymanErrorCode.UID_ERROR, e);
        }
    }
     ...
     *
     * @return UID
     * @throws LymanException in the case: Clock moved backwards; Exceeds the max timestamp
     */
    protected synchronized long nextId() {
        ...
        if (currentSecond < lastSecond) {
            ...
            throw new LymanException(LymanErrorCode.UID_ERROR,
    ...
    private long getCurrentSecond() {
        ...
        if (currentSecond - epochSeconds > bitsAllocator.getMaxDeltaSeconds()) {
            throw new LymanException(LymanErrorCode.UID_ERROR,
                    String.format("Timestamp bits is exhausted. Refusing UID generate. Now: " + currentSecond));
        }
    ...
    public void setEpochStr(String epochStr) {
        if (StringUtils.isNotEmpty(epochStr)) {
